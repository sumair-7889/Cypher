...
class PermissionService {
  ...
  Future<bool> checkAccessibility() async {
    // This would be checked via platform channel in real implementation
    // For now, return false - user must enable manually
    return false;
  }
  ...
  Future<Map<String, PermissionStatusInfo>> getAllPermissionsStatus() async {
    return {
      ...
      'overlay': PermissionStatusInfo(
        ...
        status: await checkOverlay() ? PermissionStatus.granted : PermissionStatus.denied,
        required: false,
      ),
      'accessibility': PermissionStatusInfo(
        name: 'Accessibility Service',
        status: await checkAccessibility() ? PermissionStatus.granted : PermissionStatus.denied,
        required: true,
      ),
  ...
  Future<bool> allCriticalPermissionsGranted() async {
    ...
    for (final entry in statuses.entries) {
      if (entry.value.required && entry.value.status != PermissionStatus.granted) {
        return false;
      }
    }
    return true;
